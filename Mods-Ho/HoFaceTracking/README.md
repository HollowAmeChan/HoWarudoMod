...
| 节点（代码） | 面板标题 | 状态 | 干什么 |
|---|---|---|---|
| `Nodes/HoFaceReceiverStatusNode.cs` | Ho Face 接收器（VTS 手机） | **正式** | 收来源的数据：手机（UDP 直连）或**本机的 VB**（VTS 服务端模式）；Connect / Disconnect + `原始值`（字典）+ `新鲜` + 一行 `状态`（**3 个输出口**） |
...
* 输出（**3 个**）：
  * `原始值`（`Dictionary<string,float>`，**列表语义 → 单独一个口**，喂参数处理）；
  * `新鲜`（布尔，喂参数处理的「输入新鲜」；断流回中性靠它 —— 这是信号，不是给人看的）；
...
* `配置文件` 留空 = **用内置默认**（两种内置协议的输入行 + 52 个 `ARKit/` 直通 + 4 根眼睑轴），不是"必填"。
