...
* 线名是**设备发来的原样**，而**实测这台设备发的是混合命名**（2026-09-25 拿到完整 65 键，逐条抄在这里）：
...
**① 一份透明的调试配置 `ho-debug.hoface.json`**（`Profiles/` 里有一份，沙箱里也放了一份）：
...
4. 「HoFace参数处理」：`输入新鲜` ← 接收器「新鲜」，`原始值` ← 接收器「原始值」（**输入口从上到下就是这个顺序**），
   `配置文件` 填 **`ho-debug.hoface.json`**（留空 = 这一层不做事，见 §1.1）。
