...
* 输入：`角色`(10) —— 要写的角色，Hub 挂在**他的子层级**里；`动态参数`(20) ← 控制求解的 `动态参数`。
* 按钮：`重找 Hub`（`[Trigger(200)]`）—— 丢掉缓存，下一帧重新在角色层级里找（改完角色结构不用重启）。
* 输出：`写入数` / `状态`。**`状态` 是这个节点唯一的报错出口**（它没有 flow 口，没法抛异常）。
* ⚠️ **本节点绝不会替你建 Hub**（2026-09-26 定）：角色上没有就只在 `状态` 里报一句，
  告诉你去**角色 mod** 里加一个空物体（约定叫 `SemanticHub`）挂上 `HoFaceSemanticHub`、并指向那份资产。
  理由：往用户的角色上自动加组件是"改他的东西"；而且自动建出来的那个**没挂资产** ⇒ 只有下标、没有名字，
  只会让后面更难查。（Unity 侧调试面板同一条口径：**只找不建**。）
* 键的两种形态（`Resolve`）：`#<下标>`（硬约定，不靠名字）或语义名（按**目标 Hub 的资产**查下标）；
  两头都不中 ⇒ **跳过**，而且**不是写 0** —— "没声明这个槽"和"显式写 0"是两件事。
* Hub 没挂资产时**只认 `#<下标>`**：控制器发语义名会整批被跳过，`状态` 里会点名这件事。
...
8. **跨仓**：本 Mod 只承载这一系列的**运行期**一半，另一半（词表 / 中间层主本 / 编辑器面板 /
   `*.hoface.json` / 语义 Hub 与资产）在 Unity 包 `D:\Unity_Fork\HoUnityTools`（**另一个仓库**）。
