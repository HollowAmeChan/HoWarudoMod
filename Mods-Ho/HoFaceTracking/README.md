...
| `Nodes/HoFaceHubWriteNode.cs` | HoFace写动态参数 | **正式** | 把**中间层那份参数**（参数处理的 `参数` / 「Ho合并字典」的 `字典`）写进**角色身上**的 `HoFaceSemanticHub`（先找到角色的 `HoFaceSemanticConnector`；名字按名字走，**没有表**）（2 个输出口：`写入数` / `状态`） |
...
* 输入：`角色`(10) —— 要写的角色，**Connector（以及它指向的 Hub）**挂在**他的子层级**里；
  `动态参数`(20) ← **参数处理的 `参数`**（或「Ho合并字典」的 `字典`，想覆盖几个值就走它）。
  ⚠️ **这里写的就是中间层算出来的那份参数**，跟"控制器里有没有这个参数名"无关（那是控制求解那一支的事）。
* 按钮：`重找 Connector`（`[Trigger(200)]`）—— 丢掉缓存，下一帧重新在角色层级里找（改完角色结构不用重启）。
* 输出：`写入数` / `状态`。**`状态` 是这个节点唯一的报错出口**（它没有 flow 口，没法抛异常）。
* ⚠️ **本节点绝不会替你建任何东西**（2026-09-26 定）：角色上没有 Connector、或 Connector 没填 Hub，
  都只在 `状态` 里报一句，告诉你去**角色 mod** 里加一个空物体（约定叫 `SemanticHub`）挂上
  `HoFaceSemanticHub`（存值；槽是写的人运行期按名字开的，**不需要预留长度**）与
  `HoFaceSemanticConnector`（**只是一根把手**：指向 Hub + 按名字读/写，**没有表**）。
...
18 个 `.cs` = **10 份从 HoUnityTools 包同步过来** + 8 份这里自己写
...
4. ⚠️ **这一系列是跨仓功能：这里绿了不等于验过**。本 Mod 和 Unity 包
   ...
   求值器那 10 份是**从包里搬过来的**（`Core/PORTED.md`）。所以：
   * 包侧改了那 10 份中的任何一份 ⇒ 必须**先跑同步**（`.research/sync-modcore.ps1`），
...
8. **跨仓**：本 Mod 只承载这一系列的**运行期**一半，另一半（词表 / 中间层主本 / 编辑器面板 /
   `*.hoface.json` / **动态参数那对**：Hub + Connector）在 Unity 包 `D:\Unity_Fork\HoUnityTools`（**另一个仓库**）。
   两条会静默出错的规矩：**改包侧那 10 份不跑同步** ⇒ 这边语义是旧的而编译依然过；
