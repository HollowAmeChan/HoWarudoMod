...
* 输出（**新的一条链**）：`动态参数`（字典）—— 控制器曲线写进**影子 Hub** 的语义槽，我们按下标采出来
  （键是 `#0`/`#1`… —— **影子那份 Hub 不认识名字**，名字在角色那边的 Connector 槽表上）。
...
* 输出（诊断，就这一个）：`状态` —— 多行：`参数 N 个键 · 形状 N 个 · 动态参数 N 个 · 有脸=是/否 · 头姿 (x, y, z)°`，
  ...
  （影子 Hub 有几个槽 / 有没有它；槽位是**预留**的，所以这里报的是整片槽的键数），
  控制器就绪时再加 `写入 …` 与 `自检 …`。
...
* ⚠️ **本节点绝不会替你建任何东西**（2026-09-26 定）：角色上没有 Connector、或 Connector 没填 Hub，
  ...
  `HoFaceSemanticHub`（存值，预留 128 个槽）与 `HoFaceSemanticConnector`（槽表 key/note + 指向 Hub）。
  理由：往用户的角色上自动加组件是"改他的东西"，而且自动建出来的那个**没有槽表** ⇒ 只有下标、没有名字，
  只会让后面更难查。（Unity 侧调试面板同一条口径：**只找不建**。）
* 键的两种形态（`Resolve`）：`#<下标>`（硬约定，不靠名字）或语义名（按**角色的槽表**查下标）；
  两头都不中 ⇒ **跳过**，而且**不是写 0** —— "没声明这个槽"和"显式写 0"是两件事。
  下标越界同样按"跳过"计数（`SetFloat` 越界是**静默的**，所以在这一层拦住）。
* 槽表是空的时候 = **纯位置模式**（只认 `#<下标>`），`状态` 里会明说 —— 那是合法用法，不是配错了。
* ⚠️ 控制器按**预留的 128 个槽**采样（影子 Hub 不认识名字），所以本节点每帧会写 128 个值（大多数恒为 0）。
  唯一会静默出错的错配是"**槽表没声明、但控制器写了非零值**"⇒ 这种单独一行点名并列出下标
  （只数非零，否则每帧刷屏）。
...
8. **跨仓**：本 Mod 只承载这一系列的**运行期**一半，另一半（词表 / 中间层主本 / 编辑器面板 /
   `*.hoface.json` / 语义 Hub 与 Connector 两个组件）在 Unity 包 `D:\Unity_Fork\HoUnityTools`（**另一个仓库**）。
