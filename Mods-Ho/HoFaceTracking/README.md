...
| `Nodes/HoFaceHubWriteNode.cs` | HoFace写动态参数 | **正式** | 把 `动态参数` 出口写进**角色身上**的 `HoFaceSemanticHub`（先找到角色的 `HoFaceSemanticConnector`，它的槽表解释下标）（2 个输出口：`写入数` / `状态`） |
...
* 输出（**新的一条链**）：`动态参数`（字典）—— 控制器里的**语义写手**
  （`HoFaceSemanticWriterBehaviour`，状态机行为）按**名字**开槽写进影子 Hub，我们采出来时**键就是那些名字**
  （整片槽都没名字时才退回 `#0`/`#1`；名字是写的人当场声明的，不是配置）。
  交给「HoFace写动态参数」按**角色的 Connector 槽表**把名字翻译成槽位。⚠️ 它跟形状/骨骼**是独立的**：
...
* 输出（诊断，就这一个）：`状态` —— 多行：`参数 N 个键 · 形状 N 个 · 动态参数 N 个 · 有脸=是/否 · 头姿 (x, y, z)°`，
  ...
  （影子 Hub 有几个槽、有几个有名字 / 有没有它），控制器就绪时再加 `写入 …` 与 `自检 …`。
...
* ⚠️ **本节点绝不会替你建任何东西**（2026-09-26 定）：角色上没有 Connector、或 Connector 没填 Hub，
  ...
  `HoFaceSemanticHub`（存值；槽是控制器里的写手按名字开的，**不需要预留长度**）与
  `HoFaceSemanticConnector`（槽表 key/note + 指向 Hub）。
  理由：往用户的角色上自动加组件是"改他的东西"，而且自动建出来的那个**没有槽表** ⇒ 只有名字没人认领，
  只会让后面更难查。（Unity 侧调试面板同一条口径：**只找不建**。）
* 键的两种形态（`Resolve`）：语义名（按**角色的槽表**查下标）或 `#<下标>`（纯位置，硬约定）；
  两头都不中 ⇒ **跳过**，而且**不是写 0** —— "没声明这个槽"和"显式写 0"是两件事。
  下标越界同样按"跳过"计数（`SetFloat` 越界是**静默的**，所以在这一层拦住）。
* ⚠️ **最可能出的错是"名字差一个字符"**：控制器写手填的 `slot` 与角色槽表里的 `key` 对不上时，
  那个语义**永远不动、而且完全不报错** ⇒ 本节点把跳过的键**点名**列在 `状态` 里（最多 4 个）。
* 槽表是空的时候 = **纯位置模式**（只认 `#<下标>`），`状态` 里会明说 —— 那是合法用法，不是配错了。
...
19 个 `.cs` = **11 份从 HoUnityTools 包同步过来** + 8 份这里自己写
...
4. ⚠️ **这一系列是跨仓功能：这里绿了不等于验过**。本 Mod 和 Unity 包
   ...
   求值器那 11 份是**从包里搬过来的**（`Core/PORTED.md`）。所以：
   * 包侧改了那 11 份中的任何一份 ⇒ 必须**先跑同步**（`.research/sync-modcore.ps1`），
...
8. **跨仓**：本 Mod 只承载这一系列的**运行期**一半，另一半（词表 / 中间层主本 / 编辑器面板 /
   `*.hoface.json` / 语义三件套：Hub + Connector + **写手**）在 Unity 包 `D:\Unity_Fork\HoUnityTools`（**另一个仓库**）。
   两条会静默出错的规矩：**改包侧那 11 份不跑同步** ⇒ 这边语义是旧的而编译依然过；
