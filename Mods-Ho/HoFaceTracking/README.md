...
* 键的两种形态（`Resolve`）：**语义名**（角色 Hub 上已有就复用、没有就当场开一格 —— 名字是控制器里的
  写手声明的，本节点只照抄）或 `#<下标>`（硬约定，把 Hub 开到那个长度再写）。
  键为空 ⇒ **跳过**，而且**不是写 0** —— "没声明这个槽"和"显式写 0"是两件事。
* ⚠️ **槽表 2026-09-26 删掉了** ⇒ 没有"名字对不上"的校验点了：写手把 `MouthX` 敲成 `Mouthx` 时，
  那条值会**安静地**开出新槽。所以本节点把**新声明**的名字点名列在 `状态` 里（最多 6 个）——
  那是"控制器到底声明了什么名字"的唯一记录。要查有哪些名字：跑一遍看 Hub，或看这行日志。
* 一帧里既没有名字、也没有 `#下标` ⇒ 一个槽都开不出来，`状态` 里会明说。
