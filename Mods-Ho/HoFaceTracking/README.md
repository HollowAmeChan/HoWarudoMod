...
* 它是什么：**透明直通 + 两种命名都收** —— 117 个输入行（52 个规范名 × 2 种方言：iFacialMocap 的 `jawOpen`/`mouthSmile_L`… 与
  VTS 的 `JawOpen`/`MouthSmileLeft`…，**同名最后一行生效**；
...
* ⚠️ **同名多行的语义（2026-09-25 修过一个真 bug）**：一格规范名可以有多行，求值时**每帧取「最后一个真有值的行」**，
  而不是"最后一行无条件生效"。以前是后者 ⇒ 安卓命名那行有数据、iPhone 命名那行没数据时，**没数据的那行把有数据的顶掉**，
  输入行全部缺键、值恒 0；症状极具误导性：**只有 head 系（单行）和眨眼在动** ——
  因为安卓 payload 里恰好也有 PascalCase 的 `EyeBlinkLeft`，它那行是活的。**"同名多行"就是方言兼容层，绝不该让没数据的方言赢。**
