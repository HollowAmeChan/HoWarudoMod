...
| `Nodes/HoStringFloatNode.cs` | HoStringFloat | **正式（通用件）** | **一个 `(名字, 值)` → 只含这一项的字典**（"元组"在图里的形态；没有元组端口类型，唯一的容器是字典）（`字典` / `状态`） |
| `Nodes/HoStringFloatAppendNode.cs` | HoStringFloatAppend | **正式（通用件）** | `表` + `名字` + `值` → **追加后的表**（同名时这一项赢）。官方同形的是 `Float List Add Element`（`字典` / `状态`） |
...
**HoStringFloatMerge `3f0c7d51-6a24-4f8b-9c02-8e1d5b7a64c3`**、
...
**`HoStringFloat*` 家族端口**（`Nodes/HoStringFloat*Node.cs`，2026-09-27 加；前缀一致、面板里排在一起）

| 端口 | 说明 |
|---|---|
| **`HoStringFloatDict`**：`行`(10)（`HoStringFloatEntry[]`） | **面板上手填的 `(名字, 值)` 行**，加/删行、不用接线。空名字的行忽略；同名多行 ⇒ **后面的赢** |
| **`HoStringFloat`**：`名字`(10) + `值`(20) | 一个 `(名字, 值)` —— 输出**只含这一项的字典**（= "元组"在图里的形态）。名字留空 ⇒ **空表**（`状态` 里明说） |
| **`HoStringFloatAppend`**：`表`(10) + `名字`(20) + `值`(30) | 给 `表` **追加一项**（同名时这一项赢）；不接 `表` 就是空表起步 |
...
| `状态`(输出) | 各节点自己的读数（行数 / 键数 / 覆盖了没有 / 名字为空…） |
...
典型接法：`HoStringFloatDict（手填 Ho/Drive/Gate/Lip = 0）→ 字典` ⇒ 喂「HoStringFloatMerge」的 `覆盖`，
...
**`HoStringFloatMerge` 端口**（`Nodes/HoStringFloatMergeNode.cs`，2026-09-26 加；**2026-09-27 收成两个输入口**）
...
**就这两个输入口**（2026-09-27 用户定）：不加"只补缺失"开关、也不在它里面放手填行 ——
手填是「HoStringFloatDict」的事（那才是通用件），合并只干合并。**没有"删除键"这个动作**，也不需要：想关就写 0。

典型接法：`参数处理.参数 → 基础`、`HoStringFloatDict（手填 Ho/Drive/Gate/Lip = 0）→ 覆盖`、
`字典 → 控制求解.参数`（也可以再喂「HoFace写动态参数」）。

⚠️ **覆盖的是"参数值"，所以门控请只做一层**（2026-09-26 定的边界）：拿它关门控时，
改的是控制器里那个 gate 参数（例如 `Ho/Drive/Gate/Eye`），只有在**树里门控只有一层**时，
"这个参数值"才等于"树实际在用的有效门控"。门控在树里相乘 / 嵌套过的话，有效值只活在树里，
Hub 与下游都拿不到真值 —— 要"两个开关的与"请在**中间层**乘好再喂一棵树。
完整论述见 HoUnityTools `docs/FACE_TRACKING_CONTROLLER_STRUCTURE.md` §3.3。
...
**为什么"手填"必须是一个节点（`HoStringFloatDict`）而不是合并节点里的一个字段**（2026-09-26 取证：官方场景 json 的
`typeKind` 统计 + 两套程序集全量反射）：`Dictionary<string,float>` 落在 **Reference** 类，官方图里 **8 个字典口全是 null**
...
全局 port 词汇表里**没有** `Tuple`/`KeyValuePair`，也没有第二种 `Dictionary<,>`。
**所以手填只能做成节点**：`HoStringFloatDict` 输出**字典**，于是"类型完全相同直接插"成立，
...
* `配置文件` 留空 = **这一层不做事**（**没有内置默认** —— 2026-09-25 按用户要求去掉；
  2026-09-26 起连那份内置默认表**本身**也删了，所以"没有默认"是物理事实，不是策略）。
...
* ⚠️ **`动态参数` 那个出口 2026-09-26 删掉了**（当时它是第 7 个口）。它当时采的是**影子 rig 上的
  `HoFaceSemanticHub`** —— 值由控制器里的状态机行为（「语义写手」）写进去。那套东西整个删了：
  **动态参数由中间层算完直接写角色 Hub**（Unity 侧会话，Warudo 侧就是下面那个「写动态参数」节点），
  控制器只负责"参数 → 形状 / 骨骼"。所以现在的接法是：
  `参数处理.参数 →（可选：HoStringFloatMerge）→ ┬→ 控制求解.参数`、`└→ 写动态参数.动态参数`。
* ⚠️ **没有控制器就不吐任何输出**（2026-09-25 用户定，见 §1.1.3）：`控制器` 留空、或载入失败时，
  所有口**全中性**（`BlendShapes` 空字典 / 位置零 / 骨骼 identity / `Is Tracked` 假），
...
**写动态参数节点端口**（`HoFaceHubWriteNode.cs`）

* 输入：`角色`(10) —— 要写的角色，**Hub（`HoFaceSemanticHub`）**挂在**他的子层级**里；
  `动态参数`(20) ← **参数处理的 `参数`**（或「HoStringFloatMerge」的 `字典`，想覆盖几个值就走它）。
  ⚠️ **这里写的就是中间层算出来的那份参数**，跟"控制器里有没有这个参数名"无关（那是控制求解那一支的事）。
* 按钮：`重找 Hub`（`[Trigger(200)]`）—— 丢掉缓存，下一帧重新在角色层级里找（改完角色结构不用重启）。
* 输出：`写入数` / `状态`。**`状态` 是这个节点唯一的报错出口**（它没有 flow 口，没法抛异常）。
* ⚠️ **本节点绝不会替你建任何东西**（2026-09-26 定）：角色上没有 Hub，
  就只在 `状态` 里报一句，告诉你去**角色 mod** 里加一个空物体（约定叫 `SemanticHub`）挂上
  `HoFaceSemanticHub`（一格一个「名字 + 值」；格是写的人运行期按名字开的，**不需要预留长度**）。
  理由：往用户的角色上自动加组件是"改他的东西"，而且自动建出来的那个什么都没有，只会让后面更难查。
  （Unity 侧调试面板同一条口径：**只找不建**。）
* 键就是**名字**（= 中间层输出行的 `parameter`）：角色 Hub 上已有就复用、没有就当场开一格。
  键为空 ⇒ **跳过**，而且**不是写 0** —— "没这个参数"和"显式写 0"是两件事。
  （`#<下标>` 那条老式兼容路 2026-09-26 跟着 `Hub.Reserve` 一起删了：键永远来自中间层，不来自位置。）
* ⚠️ **槽表 2026-09-26 删掉了**（连"把手"组件 `HoFaceSemanticConnector` 一起，2026-09-27）⇒ 没有"名字对不上"的校验点了：名字敲错一个字符，
  那条值会**安静地**开出新槽。所以本节点把**新声明**的名字点名列在 `状态` 里（最多 6 个）——
  那是"这一帧到底声明了什么名字"的唯一记录。要查有哪些名字：跑一遍看 Hub，或看这行日志。
* 一帧里一个名字都没收到 ⇒ 一格都开不出来，`状态` 里会明说。
...
控制器模式下 `BlendShapes` **只来自代理网格**，所以"某个形状恒 0"有三个**互相独立**的原因：
①**参数没写进 Animator**（控制器里根本没这个名字）②**控制器没把那格推到网格上**
③**网格上根本没有那个形状名**（`GetBlendShapeWeight` 是按名字取的；写了 `blendShape.foo`
而网格上没有 `foo`，Unity **不报错**，那一格永远 0）。
只报个数时前两条分不清 —— `mouthSmileLeft` 那次就是卡在这儿（现场见 §1.6）；第三条要另给证据。

⚠️ 所以载入时还会各写**一行日志**（`状态` 口要接线才看得见，日志不用 —— 2026-09-25 的教训：
`状态` 多了一行用户却没看见，白等一轮）：
...
17 个 `.cs` = **9 份从 HoUnityTools 包同步过来** + 8 份这里自己写
...
* ~~沙箱里一份都没有时，插件会写一份内置默认当样板~~ **2026-09-25 去掉**：不再自动写任何东西
  （"删了它又自己冒出来"是坑）。
* ⚠️ **现在整个仓库里没有任何"默认配置"**（2026-09-26）：连那份内置默认表本身都删了
  （包侧 `HoFaceMiddlewareDefaults` 整个类，连 `WriteDefaults()` 与"新建配置"的默认文件名一起）。
  沙箱里若还躺着 `ho-2d-test1.hoface.json`，那是**以前自动写的旧样板**（iFacialMocap 那套，已过时），
  不需要就自己删；删掉之后不会再生成，**也不会有别的东西替它兜底**
...
* 线名是**设备发来的原样**，而**实测「安卓版 VTS」发的是混合命名**（2026-09-25 拿到完整 65 键，逐条抄在这里）：
  ...
  ⚠️ ~~内置默认表给每个规范名配两行~~ —— **那张表 2026-09-26 已整个删掉**（仓库里不允许有默认配置）。
  当时的理由值得记着：这台设备的 payload 是**混合方言**（形态键走 iFacialMocap 那套 `_L/_R`、
  标量走 VTS 那套 `Rotation_x`），所以想"一张表全兼容"就会写成每个规范名两行；
  而**仓库里没有任何一张表**之后，方言兼容只能落在**具体配置**里 —— 要接这台设备就照它写一份单方言配置
  （`ho-debug-android.hoface.json` 就是），别指望有默认表兜着。
* **iFacialMocap 的接收端还没有**（它是设备侧的另一套协议，端口/握手都不同）：但它的**线名**已经出现在这台设备的 payload 里，
  所以配置里那套 `_L/_R` 行**并不是废行**（曾经的判断是错的，见上表）。
...
* 存在的理由：旧的**内置默认表**（当时沙箱里那份 `ho-2d-test1.hoface.json`，notes 写着 "generated by probe for
  read-path test"；**那张表与它的生成代码 2026-09-26 都已删掉**）**不适合调试** —— 它给 52 个形状各配了**两条**输入行（iFacialMocap + VTS），
...
* 安卓 VTS 的形状值是 **0..1**（实测 `jawOpen = 0.069`、`mouthSmile_L = 0.164`）⇒ 这份配置的形状行**不乘 0.01**；
  ⚠️ 而**已删的那份内置默认表**里，iFacialMocap 那套行写的是 `* 0.01`（那是按 iFacialMocap **App** 的 0..100 写的）——
  当年拿它接安卓 VTS 会小 100 倍。**这就是"默认表"的典型危害**：它按另一台设备的量纲写死了换算，
  用错了不报错、只是小 100 倍（2026-09-26 那张表已整个删掉）。
...
同步收缩了 `NodeTypes`（当时 **5 个** —— 后来陆续加了通用件：2026-09-26「Ho合并字典」→ 6 个，2026-09-27 `HoStringFloat*` 家族与 `HoBool2Float` → **10 个**，`HoFaceTrackingPlugin.cs` 的 `NodeTypes`）：漏列的节点即使编译进程序集
...
* **热更新会掐断已连上的接收器**（用户实测：Build 完 Warudo 热更新 → 接收器断连，"重新点连接"绑不上、
  ...
  ⚠️ **热更新之后一定要重新点一次 Connect**：接收器关掉时下游**不会报错** ——
  中间层照样输出整张键表、只是**每个值都是 0**（"缺键 = 不写这一行、保持上一帧"，初始值就是 0），
  看起来像"同样的配置突然输出不出来了"。判据：`[Ho 面捕] 接收器 运行中=False 状态=未启动` 之后
  再没有新的接收器行、且 `Connect（…）→ …` 那行没出现过；参数处理的 `状态` 会是 `原始键 0（不新鲜）`
  （来了但全是 0 时是 `原始键 90（新鲜）`）。完整对照表见 HoUnityTools
  `docs/pitfalls/BUILD_AND_TOOLING.md` §9.1。
...
4. ⚠️ **这一系列是跨仓功能：这里绿了不等于验过**。本 Mod 和 Unity 包
   `D:\Unity_Fork\HoUnityTools`（**另一个仓库**，`com.hollow.hounitytools`）是同一功能的两半，
   求值器那 9 份是**从包里搬过来的**（`Core/PORTED.md`）。所以：
   * 包侧改了那 9 份中的任何一份 ⇒ 必须**先跑同步**（`.research/sync-modcore.ps1`），
     否则这边还是旧语义，**而且编译照样过** —— 表现是"Warudo 里和面板里不一样"；
   * 在 `Core/` 副本里直接改 ⇒ 下次同步**无声覆盖**；
   * 两边各有**只有它有**的那一关：这边是 `UMod sandbox lint`，包侧是离线用例 + 批处理 Unity 用例 +
     整包编译检查。**两边都过才算验过**（完整规矩：包侧 `docs/PARAMETER_HO.md` §0.0、
     `docs/pitfalls/REPO_AND_GIT.md` §7）。
...
7. `[PluginType]` 的 **`NodeTypes` 必须列全**，漏掉的节点不会出现在面板里
   （`docs/打包与脚本规范.md` §3、§9）。
8. **跨仓**：本 Mod 只承载这一系列的**运行期**一半，另一半（词表 / 中间层主本 / 编辑器面板 /
   `*.hoface.json` / **动态参数 Hub**）在 Unity 包 `D:\Unity_Fork\HoUnityTools`（**另一个仓库**）。
   两条会静默出错的规矩：**改包侧那 9 份不跑同步** ⇒ 这边语义是旧的而编译依然过；
   **直接改 `Core/` 副本** ⇒ 下次同步覆盖。清单与"改了哪一层两边各跑什么"见 §4 第 4 条、
   `Core/PORTED.md`、包侧 `docs/PARAMETER_HO.md` §0.0。
9. **Editor 专用脚本不在 `compile-check` 范围内**（它只编 `Mods/` 与 `Mods-Ho/` 里的运行时代码），
