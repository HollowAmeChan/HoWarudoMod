...
（2026-09-26 起这 9 份里**已经没有 `ScriptableObject` 了** —— 语义词表从资产改成了组件，
后来连组件也只剩 Hub 一个，见上）；
...
**2026-09-26 实测：这 9 份与"重跑一遍同步"的输出逐字节一致**
（用脚本的同一套规则——去 BOM、换命名空间、归一成 LF、拼上头——重算一遍再比字节；
9 份全 identical，包侧主本也是无 BOM UTF-8）。所以现在两边没有分叉。
...
脚本会：重新拷贝这 9 份 → 换命名空间 → 加标记头 → 写成**不带 BOM 的 UTF-8 + LF**
...
求值、缺键语义、修饰符顺序、曲线端点夹取仍由上面那 9 份（与包同源）实现。
