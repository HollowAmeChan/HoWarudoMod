...
## 2. 这里自己写的 4 份（**不在**同步清单里）

| 文件 | 干什么的 |
|---|---|
| `HoFaceChain.cs` | 中间层的**运行期求值器**（Warudo 侧）：没有 Animator、没有代理渲染器、没有通道模式，纯求值 |
| `HoFaceInputState.cs` | 最近一帧的共享状态 + 接收器统计（接收器写、节点读，唯一交接点） |
