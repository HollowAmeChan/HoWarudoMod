// HoStringFloatAppendNode.cs  --  「HoStringFloatAppend」：把**一个元组**并进一张表
//
// 【形状】`表`(Dictionary<string,float>) + `元组`(`KeyValuePair<string,float>`) → `字典`（追加后的表）
//   —— 这就是用户说的"**拼成一个元组，然后再转字典**"里的**后半步**：元组进表，表就是字典。
//   （官方的同形节点是 `Float List Add Element` / `Boolean List Add Element`：`List` + 元素 → `Result`。
//     我们这里是"名字 → 浮点的表" + `(名字, 值)` 元组 → 表。）
//
...
//   ② **同名时这一项赢**（append = 后写覆盖）。
//   ③ **元组的名字为空**（`Key` 是空串/null）⇒ 这一项**不加**，原样返回输入表；`状态` 里会点名。
//
...
namespace HoFaceTracking.Nodes
{
    ...
    public class HoStringFloatAppendNode : Node
    {
        ...
        /// <summary>要并进去的那个 `(名字, 值)`。名字为空 ⇒ 不加（原样返回输入表）。</summary>
        [DataInput(20)]
        [Label("元组")]
        ...
        public Dictionary<string, float> Appended()
        {
            ...
            {
                bool exists = result.ContainsKey(Tuple.Key);
        ...
        public string Status()
        {
            Appended();
            if (!appendedLast)
                return "表 " + result.Count + " 键  ·  ⚠ 元组的名字为空 ⇒ 这一项没加（原样返回输入表）";
            return "表 " + result.Count + " 键  ·  " + Tuple.Key + " = " + Tuple.Value.ToString("0.###")
        ...
        private void LogOnce()
        {
            string state = (Table != null ? Table.Count : 0) + "/" + result.Count + "/"
                + (appendedLast ? 1 : 0) + "/" + (overwroteLast ? 1 : 0) + "/" + (Tuple.Key ?? "");
            if (state == loggedState) return;
            loggedState = state;
            Debug.Log("[Ho StringFloatAppend] 表 " + (Table != null ? Table.Count : 0)
                + " → " + result.Count + " 键"
                + (appendedLast ? "  ·  " + Tuple.Key + " = " + Tuple.Value.ToString("0.###")
                    + (overwroteLast ? "（覆盖）" : "（新增）") : "  ·  元组名字为空，没加"));
        }
    }
}
