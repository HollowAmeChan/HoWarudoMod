// HoStringFloatNode.cs  --  「HoStringFloat」：面板上手填若干 `(名字, 值)`，出来一份字典
//
// 【名字】（2026-09-27 用户定）面板标题就叫 **`HoStringFloat`** —— 它描述的是这个东西本身：
// 一串 `(string, float)`。别叫"浮点元组"那种自造词。
//
...
// 后者就是官方的"元组"机制（面板上加/删行，字段是 `[DataInput]`）。
// 所以"手填一张 (名字, 值) 表"这件事**只能做成节点**，而这个节点就是那个通用件。
//
// 【典型用法】
//   行里填 `Ho/Drive/Gate/Lip = 0` ⇒ `字典` 喂给「Ho合并字典」的 `覆盖`（或直接喂「HoFace控制求解」）
//   行里填若干常量 ⇒ 想给谁就给谁：合并字典 / 控制求解 / 写动态参数，都是同一个 `Dictionary<string,float>`。
//
// 【为什么输出是字典，而不是"元组列表"】（2026-09-27 定）
...
//   多态口（`object`）只有在"要接线的元组真的存在"时才有意义，而它并不存在（官方没有 Tuple/KeyValuePair
//   端口类型，我们也不打算把自定义 struct 当图端口 —— 那属于未验证区）。
//
...
namespace HoFaceTracking.Nodes
{
    ...
    /// </summary>
    public class HoStringFloatEntry : StructuredData<HoStringFloatNode>, ICollapsibleStructuredData
    {
    ...
    public class HoStringFloatNode : Node
    {
        ...
        private void LogOnce()
        {
            ...
            loggedState = state;
            Debug.Log("[Ho StringFloat] 行 " + (Rows != null ? Rows.Length : 0)
                + " · 有效 " + usedLast
                + " · 结果 " + table.Count + " 键"
