...
//
// 【形状】`名字`(string) + `值`(float) → `键值对`(`KeyValuePair<string,float>`)
// 这就是"把值拼成一个键值对"那一步：布尔先用 `HoBool2Float` 换成 1/0，再由这里配上名字。
//
...
// ⇒ 所以家族改成**真键值对口**：本节点吐 `KeyValuePair<string,float>`、`HoStringFloatAppend` 收它。
//
// 【家族】
//   `HoStringFloat`        —— 名字 + 值 → **键值对**（本文件）
//   `HoStringFloatAppend`  —— 表 + 键值对 → 追加后的表（**"再转字典"就是它**）
...
namespace HoFaceTracking.Nodes
{
    ...
    public class HoStringFloatNode : Node
    {
        ...
        /// <summary>这一项的名字。**留空 = 这个键值对没有名字**（下游会跳过它）。</summary>
        [DataInput(10)]
        [Label("名字")]
        public string Key;
        ...
        /// <summary>状态：这个键值对是什么（或"名字为空"）。</summary>
        [DataOutput]
        [Label("状态")]
        public string Status()
        {
            return string.IsNullOrEmpty(Key)
                ? "⚠ 名字为空 ⇒ 这个键值对没有名字（下游 `HoStringFloatAppend` 会跳过它）"
