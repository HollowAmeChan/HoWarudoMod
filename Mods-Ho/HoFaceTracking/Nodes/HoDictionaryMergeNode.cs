...
// 【典型用法：把控制器里恒 1 的门控关掉】
//   基础   ← 参数处理节点的「参数」
//   覆盖行 ← **不接线、面板上加一行**：`GateLip` = 0
//   字典   → 喂给「HoFace控制求解」（接的是它的 `动态参数` 口）
//   控制器里 `GateLip` 这种门控参数**中间层从来不写**，它的默认值住在控制器参数上（默认 1）；
//   把 0 并进字典之后，求解节点写参数时就会把 0 写进去 ⇒ 门控关掉，而且**没有一个"第二个写者"**
//   在旁边抢（覆盖是在同一次写入里改的值，不是另写一遍）。
//
